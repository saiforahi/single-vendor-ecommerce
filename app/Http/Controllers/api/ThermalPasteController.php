...
use App\Models\ThermalPaste;
use App\Models\Product;
use App\Http\Requests\v1\CreateThermalPastesRequest;
use App\Events\UploadImageEvent;
class ThermalPasteController extends Controller
{
    
    public function create(CreateThermalPastesRequest $req){
        try{
            $new_product = Product::create($req->only('price'));
            $new_thermalpaste = ThermalPaste::create(array_merge($req->except('total_images'),['product_id'=>$new_product->id]));
            $images=array();
